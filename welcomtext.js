const welcome = (number, groupname) => {
    return `Hola! @${number}. Bienvenido a ${groupname} para permanecer en el grupo *preséntate con nombre, edad y lugar* recuerda leer las normas de la descripción. *protegido por:* 
*L̴͕̰̠̤̃̒̒ḛ̵̠̟̠͆̓̎n̴̘͈͊͛͗͝a̶̛͓͊ A̵n̴o̶n̷y̶m̸i̶s̴s̶      ⪻̤̈N̴͖e̵͠b̷̛̈́á̴̬̌l̶͐́o̸͡⪼̤̈*
⠀⠀⠀⠀*⪻̤̈M̸̞̱̜͒̈͑̐̈́̇•̴̻̖̼͓͈̀̈́̈̓̍̔̚ͅT̴̤̩̀͑̄͗̒͑̕ḩ̸͇͔̑͒́͌̄e̷̙̱̎•̷̔̓̉̽̑̕̕H̶͔̰͑å̷͙̹̾͝͝ť̵̎͐́̏̒̈ë̶̛͖͓̙͎́́ṛ̷̊̊⪼̤̈*`
}
exports.welcome = welcome

const bye = (number) => {
    return `Adiós @${number}. Esperamos no volver a verte!! 👋`
}
exports.bye = bye
